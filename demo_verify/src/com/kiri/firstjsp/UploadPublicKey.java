...
/**
 * Servlet implementation class UploadPublicKey
 */
@WebServlet("/function/uploadPublicKey")
public class UploadPublicKey extends HttpServlet {
	private static final long serialVersionUID = 1L;
	
	private static final String UPLOAD_DIR = "upload\\publickeys";
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		request.getRequestDispatcher("/WEB-INF/ProtectedFile/uploadpublickey.jsp").forward(request, response);
	}
		...
		try {
            ...
            if (formItems != null && formItems.size() > 0) {
                for (FileItem item : formItems) {
                    if (!item.isFormField()) {
                        String fileName = (String) session.getAttribute("username") + ".pub.asc";
